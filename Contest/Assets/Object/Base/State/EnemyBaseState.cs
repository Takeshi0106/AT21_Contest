        ...
        foreach (var info in collidedInfos)
        {
            ...
            if (info.multiTag.HasTag(playerAttackTag))
            {
                ...
                if (isCounterAttack)
                {
                    ...
                    if (playerState.GetPlayerCounterManager() == null)
                    {
                        Debug.Log("EnemyBaseState::HandleDamage() : PlayerCounterManagerが見つかりません");
                    }

                    Debug.Log($"EnemyBaseState::HandleDamage() : CounterDamage : {playerState.GetPlayerCounterManager().GetCounterDamage()}");
                    Debug.Log($"EnemyBaseState::HandleDamage() : PlayerMultiplier : {playerState.GetPlayerCounterManager().GetDamageMultiplier()}");
                }
                // 投げる攻撃の場合の処理
                else if (isThrowAttack)
                {
                    ...
                    if (throwState == null)
                    {
                        Debug.LogError("throwState : 見つかりません");
                    }

                    baseDamage = throwState.GetThrowDamage(); // ダメージを取得
                    Debug.Log($"オブジェクトがぶつかった　:　{baseDamage}");
